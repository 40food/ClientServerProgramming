...
//        super.addStudent(request, responseObserver);
        logging(request.getRequestId(),"<학생 추가>", Instant.now());
        try{
            studentList=new StudentList(studentPath);
            boolean result= studentList.addStudentRecord(studentPath,request.getPayload());
            if(result) {
...
//        super.deleteStudent(request, responseObserver);
        logging(request.getRequestId(),"<학생 삭제>", Instant.now());
        try{
            studentList=new StudentList(studentPath);
            boolean result= studentList.deleteStudentRecord(studentPath,request.getPayload());
            if(result) {
                Response response = Response.newBuilder()
                        .setMessage("학생 삭제 성공")
                        .build();
