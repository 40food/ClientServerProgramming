...
//        super.request(request, responseObserver);
        try {
            ...
            switch (type) {
                case "학생 추가하기": response = addStudent(id,payload,stub);break;
                case "학생 삭제하기": response = deleteStudent(id,payload,stub);break;
                default: break;
            }
...
//        super.multiRequest(request, responseObserver);
    ...
    private static Response addStudent(String id, String payload, DataServiceGrpc.DataServiceBlockingStub stub){
        Response response=stub.addStudent(Request.newBuilder()
                .setRequestId(id)
                .setType("학생 추가하기-server")
                .setPayload(payload)
                .build());
        return Response.newBuilder().setMessage(response.getMessage()).build();
    }
    private static Response deleteStudent(String id, String payload, DataServiceGrpc.DataServiceBlockingStub stub){
        Response response=stub.deleteStudent(Request.newBuilder()
                .setRequestId(id)
                .setType("학생 삭제하기-server")
                .setPayload(payload)
                .build());
        return Response.newBuilder().setMessage(response.getMessage()).build();
    }
