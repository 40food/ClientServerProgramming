...
/**뭔가 안정성이 떨어지는 거 같은데...?
 * 1. 실행에 순서가 있는 것 같음. 순서 잘못되면 이벤트 전달 안 됨?(요청 속도의 문제일수도)
 * 2. 가끔 이벤트를 전송했는데 전달이 안 되고 사라짐(이게 제일 문제)
 *
 * 장점
 * 1. 단일 이벤트 처리=비동기적으로 검증 가능
 * 2. 효율적인 데이터 구조
 * 3. 확장성
 * */
...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "6":
					...
					break;
				case "7":
					eventBus.sendEvent(new Event(EventId.Registration, setRegistration()));
					printLogSend(EventId.Registration);
					break;
				case "0":
	...
	private static String setRegistration() throws IOException {
		System.out.println("\nEnter student ID and press return (Ex. 60211663)>> ");
